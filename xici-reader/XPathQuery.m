...
NSDictionary *DictionaryForNode(xmlNodePtr currentNode, NSMutableDictionary *parentResult,BOOL parentContent)
{
  ...
  if (currentNode->name)
    {
      NSString *currentNodeContent =
        [NSString stringWithCString:(const char *)currentNode->name encoding:kENC];
  ...
  if (currentNode->content && currentNode->content != (xmlChar *)-1)
    {
      NSString *currentNodeContent =
        [NSString stringWithCString:(const char *)currentNode->content encoding:kENC];
      ...
      while (attribute)
        {
          ...
          NSString *attributeName =
            [NSString stringWithCString:(const char *)attribute->name encoding:kENC];
          if (attributeName)
            {
...
NSArray *PerformXPathQuery(xmlDocPtr doc, NSString *query)
{
  ...
  xpathObj = xmlXPathEvalExpression((xmlChar *)[query cStringUsingEncoding:kENC], xpathCtx);
  if(xpathObj == NULL) {
