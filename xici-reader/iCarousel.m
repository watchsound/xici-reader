...
-(void)autoAnimatedToNext{
    if ( !_dataSource )
        return;
    if ( self.currentItemIndex >= [_dataSource numberOfItemsInCarousel:self] -1 ){
        [self scrollToItemAtIndex:0 duration:3];
    } else {
        [self scrollToItemAtIndex:self.currentItemIndex +1 duration:1];
    }
}

-(void)stopAutoAnimation{
    if (self.autoAnimationTimer ){
        [self.autoAnimationTimer invalidate];
        self.autoAnimationTimer = nil;
    }
}
...
-(void)startAutoAnimation{
    ...
    self.autoAnimationTimer = [NSTimer scheduledTimerWithTimeInterval:4.0
                                                          target:self
                                                        selector:@selector(autoAnimatedToNext)
                                                        userInfo:nil
                                                         repeats:YES];
